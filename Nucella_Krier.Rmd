...
mycolors <- c("blue3","dodgerblue2","darkorange", "darkorange4")
...
heatmap(phiST,scale="none", cexRow = 1, cexCol=0.8)
...
```{r}
#write.csv(pop,"amova_hypotheses_nucella2.csv",row.names = F, quote=F)
...
amovahyps<-read.csv("amova_hypotheses_nucella2.csv")
CapeMendocino<-amovahyps$CapeMendocino
pop1<-as.factor(pop)

#Calculate the p-distance among all sequences
dists<-dist.dna(Nostrina,model="raw")
```

Test the Cape Mendocino hypothesis:

```{r}
amova_out<-pegas::amova(formula=dists~CapeMendocino/pop1,nperm=10000)
